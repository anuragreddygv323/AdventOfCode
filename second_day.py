...
aoc_input = """
...
class Keypad(object):
	...
	def convert_coords_to_value(self, i, j):
		n = self.size
		num = (i+1) + j*n
		return num
	...
	def parse_sequence(self, sequence):
		if len(sequence) > 0:
			...
			x, y = self.coords
			number = self.convert_coords_to_value(x, y)
			return number
